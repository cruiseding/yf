...
import com.wp.yf.app.dao.UserChatDao;
...
public class MemberCenterController extends InOutManager implements InOutFace, ServletConfigAware {

	
    private ServletConfig servletConfig;
	
	public MemberCenterController(String[] arg, HttpServletRequest request, HttpServletResponse response)
			throws IOException, ServletException, SQLException {
		super(arg, request, response);
	}

	@RequestMapping("/member196")
	...
	SqlManagerFace sqlmface = new UserChatDao();
...
//				break;
	...
	private void find_qiangliao(String[] arg) throws SQLException, ServletException, IOException {
		...
		for (int i = 0; i < list1.size(); i++) {
			...
			sql = "select count(*) from blacklist_table where (user_id=" + arg[2] + " and black_id=" + arg[0]
					+ " and status=1) or (user_id=" + arg[0] + " and black_id=" + arg[2] + " and status=1)";
			log.send(DataType.basicType, "196", "快撩页面查询抢聊表内容，显示未抢聊-sql", sql);
...
//		}
	...
	/**
	 * arg[0] A-user-add arg[1] give_gift arg[2] user_id ----- 用户id arg[3] zhubo_id
	 * ----主播id arg[4] coin ----v币金额 arg[5] id ----礼物ID
	 * 
	 * @param arg
	 * @throws IOException
	 * @throws SQLException
	...
	private void give_gift(String[] arg) throws SQLException, IOException, ServletException {
		...
		if (list == null || list.isEmpty())// 没有查询到礼物数据
		{
			...
			if (yue < giftPrice) // 2.判断余额
			{
				...
				String name2 = "";
				String phote2 = "";
				name1 = list1.get(0).get("nickname").toString();
				phote1 = list1.get(0).get("user_photo").toString();
				...
				name2 = list2.get(0).get("nickname").toString();
				phote2 = list2.get(0).get("user_photo").toString();
				...
				log.send(DataType.specialType, "01165", "发送openfire消息内容 ", ids + "卍" + "send_liwu," + name1 + ","
						+ phote1 + "," + name2 + "," + phote2 + "," + gift_photo);
...
//				 });
	...
	/**
	 * 添加关注 target_id 1 添加关注之前，先查询是否已经有关联关系（有没有访问记录，为下一步做添加还是修改做判断） select * from
	 * relation_table where user_id=3(arg[2]) and target_id=4(arg[3]) -----select *
	 * from relation_table where user_id=3(arg[3]) and target_id=4(arg[2]) 3
	 * 查询--关注时间 如果有如果有关联关系了，但是关注时间为空,做修改操作 select user_follow_time from
	 * relation_table where user_id=3 and target_id=4 3.1 修改语句中添加关注时间的操作 update
	 * relation_table set user_follow_time=NOW() where user_id=3 and target_id=4
	 * 
	 * select target_follow_time from relation_table where user_id=4 and target_id=3
	 * 3.2 修改语句中添加关注时间的操作 update relation_table set target_follow_time=NOW() where
	 * user_id=3 and target_id=4 else --------if 2
	 * 如果有关系值，说明已经有关联关系过，返回查询到的值，如果没有，添加一条关注记录 insert into relation_table
	 * (user_id,target_id,user_follow_time) values (5,6,NOW())
	 **/
...
//			} else 
	...
	private void fangke(String[] arg) throws SQLException, IOException, ServletException {
		...
		if (a == 0 && b == 0) {
			...
			/*
			 * String sql1=sqlmface.searchSqlface(1, arg); list1=sqlUtil.get_list(sql1);
			 */
